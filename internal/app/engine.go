...
const roundSigDomain = "THOMAS|hdr|v1|"
...
// ---- 라운드 커밋/조회 ----
func (e *Engine) CommitRound() (hdr types.RoundHeader, ok bool) {
	e.mu.Lock()
	defer e.mu.Unlock()
	...
	// Merkle root 계산
	rootBytes := computeMerkleRoot(e.leaves)

	// 라운드 헤더 구성 (TimeUTC는 int64)
	hdr = types.RoundHeader{
		Round:      uint64(len(e.rounds) + 1),
		FromHeight: e.height,
		ToHeight:   e.height + 1,
		TxCount:    uint64(len(e.leaves)),
	...
	e.height++
	e.lastCommittedHeight = hdr.ToHeight
...
func (e *Engine) commitRoundCore() (types.RoundHeader, bool) {
	e.mu.Lock()
	defer e.mu.Unlock()
	...
	hdr := types.RoundHeader{
		...
		Root:       hex.EncodeToString(computeMerkleRoot(sub)),
		TimeUTC:    time.Now().UTC().Unix(),
	}
...
func (e *Engine) SignRoundHeader(h types.RoundHeader) (sig []byte, ok bool) {
	...
	buf := append([]byte(roundSigDomain), msg...)
	s := ed25519.Sign(e.priv, buf)
	return s, true
}
...
func computeMerkleRoot(leaves [][]byte) []byte {
	...
	for len(layer) > 1 {
		...
		for i := 0; i < len(layer); i += 2 {
			if i+1 < len(layer) {
				h := sha256.Sum256(append(layer[i], layer[i+1]...))
				b := make([]byte, 32)
				copy(b, h[:])
				next = append(next, b)
			} else {
				// 마지막 홀수 노드 복제하여 페어링
				h := sha256.Sum256(append(layer[i], layer[i]...))
				b := make([]byte, 32)
				copy(b, h[:])
				next = append(next, b)
			}
