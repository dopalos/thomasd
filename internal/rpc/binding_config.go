...
func lookupPubForFrom(from string) (string, bool) {
	...
	if v, ok := cfgMap[from]; ok && strings.TrimSpace(v) != "" {
		return strings.TrimSpace(v), true
	}
	if v, ok := os.LookupEnv("THOMAS_PUBKEY_" + from); ok && strings.TrimSpace(v) != "" {
		return strings.TrimSpace(v), true
	}
	return "", false
}
